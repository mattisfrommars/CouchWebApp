...
import framework.utils.auth.{ DefaultEnv, WithProvider }
...
class ChangePasswordController @Inject() (
  ...
  def view = silhouette.SecuredAction(WithProvider[DefaultEnv#A](CredentialsProvider.ID)) { implicit request =>
    Ok(views.html.user.changePassword(ChangePasswordForm.form, request.identity))
  }
  ...
  def submit = silhouette.SecuredAction(WithProvider[DefaultEnv#A](CredentialsProvider.ID)).async { implicit request =>
    ChangePasswordForm.form.bindFromRequest.fold(
      form => Future.successful(BadRequest(views.html.user.changePassword(form, request.identity))),
      password => {
