...
import framework.services.{UserService, UserServiceImpl}
...
import framework.utils.auth.{CustomSecuredErrorHandler, CustomUnsecuredErrorHandler, DefaultEnv}
...
class SilhouetteModule extends AbstractModule with ScalaModule {
  ...
  def configure() {
    ...
    bind[UserAuthComponent].to[UsersRepository]
    ...
    bind[DelegableAuthInfoDAO[PasswordInfo]].to[PasswordRepository]
    bind[DelegableAuthInfoDAO[OAuth1Info]].to[OAuth1InfoRepository]
    bind[DelegableAuthInfoDAO[OAuth2Info]].to[OAuth2InfoRepository]
    bind[DelegableAuthInfoDAO[OpenIDInfo]].to[OpenIDInfoRepository]
  }
