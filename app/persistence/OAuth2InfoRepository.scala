...
/**
  ...
  */
@Singleton
class OAuth2InfoRepository @Inject()(protected val dbConfigProvider: DatabaseConfigProvider)
  extends DelegableAuthInfoDAO[OAuth2Info] with AuthenticationComponent with HasDatabaseConfigProvider[JdbcProfile] {
  ...
  def save(loginInfo: LoginInfo, authInfo: OAuth2Info): Future[OAuth2Info] = {
    val query = for {
      result <- loginInfoQuery(loginInfo).joinLeft(OAuth2Infos).on(_.id === _.loginInfoId)
    } yield result
