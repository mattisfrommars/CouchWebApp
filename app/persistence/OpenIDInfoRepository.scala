...
/**
  ...
  */
@Singleton()
class OpenIDInfoRepository @Inject()(protected val dbConfigProvider: DatabaseConfigProvider)
  extends DelegableAuthInfoDAO[OpenIDInfo] with AuthenticationComponent with HasDatabaseConfigProvider[JdbcProfile] {
  ...
  protected def addAction(loginInfo: LoginInfo, authInfo: OpenIDInfo) =
    loginInfoQuery(loginInfo).result.head.flatMap { dbLoginInfo =>
      DBIO.seq(
        OpenIDInfos += domain.OpenIDInfo(authInfo.id, dbLoginInfo.id.get),
        OpenIDAttributes ++= authInfo.attributes.map {
          case (key, value) => domain.OpenIDAttribute(authInfo.id, key, value)
        })
    }.transactionally
  ...
  protected def updateAction(loginInfo: LoginInfo, authInfo: OpenIDInfo) =
    openIDInfoQuery(loginInfo).result.head.flatMap { dbOpenIDInfo =>
      DBIO.seq(
        OpenIDInfos filter(_.id === dbOpenIDInfo.id) update dbOpenIDInfo.copy(id = authInfo.id),
